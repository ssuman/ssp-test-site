...
function scoreAd(
  ...
  console.groupCollapsed(
    "%c PAAPI SSP %c scoreAd %c BID:%d => SCORE:%d",
  ...
  var debug =
    "&winningBid=${winningBid}&winningBidCurrency=${winningBidCurrency}&topLevelWinningBid=${topLevelWinningBid}&topLevelWinningBidCurrency=${topLevelWinningBidCurrency}";
  var bidDetails =
    "&owner=" +
  ...
  forDebuggingOnly.reportAdAuctionWin(
    auctionConfig.seller +
      "/ssp/forDebuggingOnly?type=reportAdAuctionWin" +
      queryString
  );
  forDebuggingOnly.reportAdAuctionLoss(
    auctionConfig.seller +
      "/ssp/forDebuggingOnly?type=reportAdAuctionLoss" +
      queryString
  );
  ...
  return {
    desirability: score,
    rejectReason:
      "not-available" /* https://github.com/qingxinwu/turtledove/blob/main/Proposed_First_FLEDGE_OT_Details.md#reporting */,
...
function reportResult(auctionConfig, browserSignals, directFromSellerSignals) {
  console.groupEnd();
  console.groupCollapsed(
    "%c PAAPI SSP %c reportResult %c BID:%d",
    "color: green;  background-color:yellow; border: 1px solid black",
    "color: blue; border:1px solid black",
    "",
    browserSignals.bid
